...
func testVendored(t *testing.T, context spec.G, it spec.S) {
	...
	context("when the node_modules are vendored", func() {
		...
		context("and the node_modules and pnpm-offline-mirror are missing dependencies required in pnpm-lock.yaml", func() {
			it("should fail to build with a helpful error", func() {
				...
				image, _, err = pack.Build.
					...
					WithNetwork("none").
					Execute(name, source)
				Expect(err).To(MatchError(ContainSubstring(`error Can't make a request in offline mode ("https://registry.npmjs.org/leftpad/-/leftpad-0.0.1.tgz")`)))
			})
		})
	})
}
