	...
	context("when service bindings are provided", func() {
		...
		context("when the binding has type npmrc", func() {
			...
			it.Before(func() {
				...
				case "darwin":
					serverURI = fmt.Sprintf("host.docker.internal:%s", uri.Port())
				case "windows":
					serverURI = fmt.Sprintf("host.docker.internal:%s", uri.Port())
				case "linux":
					// host.docker.internal is not supported on linux; use host's address
					// and build WithNetwork("host")
					serverURI = fmt.Sprintf("127.0.0.1:%s", uri.Port())
				default:
					t.Fatal("unrecognized runtime.GOOS: " + runtime.GOOS)
				}

				//				Expect(os.WriteFile(filepath.Join(source, "pnpm-lock.yaml"), []byte(fmt.Sprintf(`# THIS IS AN AUTOGENERATED FILE. DO NOT EDIT THIS FILE DIRECTLY.
				//# yarn lockfile v1
				//
				//
				//leftpad@~0.0.1:
				...
				Expect(os.WriteFile(filepath.Join(binding, ".npmrc"), []byte(fmt.Sprintf(`registry=http://%[1]s/
				 @jsr:registry=http://%[1]s/
			...
			it("successfully uses the npmrc to authenticate and fails due to invalid pnpm package", func() {
				...
				Expect(err).To(MatchError(ContainSubstring("ERR_PNPM_TARBALL_INTEGRITY")))
			})
		})
		...
		context("when binding has type pnpmrc", func() {
			it.Before(func() {
				Expect(os.WriteFile(filepath.Join(binding, "type"), []byte("npmrc"), os.ModePerm)).To(Succeed())
				Expect(os.WriteFile(filepath.Join(binding, ".npmrc"), []byte(`ignore-scripts=true`), os.ModePerm)).To(Succeed())
			})
