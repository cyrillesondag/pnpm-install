...
func testBuild(t *testing.T, context spec.G, it spec.S) {
	...
	context("when required during build", func() {
		...
		it("returns a result that installs build modules", func() {
			...
			Expect(determinePathCalls[1].Typ).To(Equal("pnpm-workspace"))
			Expect(determinePathCalls[1].PlatformDir).To(Equal("some-platform-path"))
			Expect(determinePathCalls[1].Entry).To(Equal("pnpm-workspace.yaml"))
	...
	context("failure cases", func() {
		...
		context("when determining the path for the pnpm-workspace fails", func() {
			it.Before(func() {
				configurationManager.DeterminePathCall.Stub = func(typ, platform, entry string) (string, error) {
					if typ == "pnpm-workspace" {
						return "", errors.New("failed to determine path for pnpm-workspace")
					}
					return "", nil
				}
			})
			...
			it("errors", func() {
				...
				Expect(err).To(MatchError("failed to determine path for pnpm-workspace"))
			})
		})
		...
		context("when pnpm-workspace.yaml service binding symlink cannot be created", func() {
			it.Before(func() {
				configurationManager.DeterminePathCall.Stub = func(typ, platform, entry string) (string, error) {
					if typ == "pnpm-workspace" {
						return "some-path/pnpm-workspace.yaml", nil
					}
					return "", nil
				}

				symlinker.LinkCall.Stub = func(o string, n string) error {
					if strings.Contains(o, "pnpm-workspace.yaml") {
						return errors.New("symlinking pnpm-workspace.yaml error")
					}
					return nil
				}
			})
			...
			it("errors", func() {
				...
				Expect(err).To(MatchError(ContainSubstring("symlinking pnpm-workspace.yaml error")))
			})
		})
		...
		context("when pnpm-workspace.yaml binding symlink can't be cleaned up", func() {
			it.Before(func() {
				symlinker.UnlinkCall.Stub = func(p string) error {
					if strings.Contains(p, "pnpm-workspace.yaml") {
						return errors.New("unlinking pnpm-workspace.yaml error")
					}
					return nil
				}
			})
			it("returns an error", func() {
				...
				Expect(err).To(MatchError("unlinking pnpm-workspace.yaml error"))
			})
		})
	})
}
